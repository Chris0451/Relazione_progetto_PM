...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item \textbf{PersonalDataScreen.kt:}
      Vista di sola lettura dei dati utente con \textit{TopAppBar} per tornare indietro e pulsante \textit{Modifica}. Mostra avatar, username, email e campi anagrafici attraverso la componente \textit{LabelAndPlaceholder}; lo stile usa card e colori coerenti con Material.
      \item \textbf{ModifyPersonalDataScreen.kt:}
      Form di modifica profilo e cambio password in Compose. La sezione dati usa \textit{SimpleTextField} con validazioni base e invoca \textit{onSave}. La sezione password usa \textit{PasswordTextField} con visibilità alternabile, validazioni su lunghezza e conferma, e flusso di \textit{reauthenticate} e \textit{updatePassword} tramite Firebase Authentication, mostrando stato di caricamento ed errori.
      \item \textbf{FriendsList.kt:} Schermata elenco amici con ricerca e tre schede (\textit{Amici}, \textit{Richieste}, \textit{Utenti}); carica dati tramite \textit{FriendsManager}, filtra i risultati, gestisce accettazione o rifiuto richieste e invio richiesta; tocco su un amico apre i dettagli tramite callback di navigazione; usa \textit{SearchAndTabs}, \textit{FriendItem}, \textit{RequestItem}.
      \item \textbf{FriendDetailScreen.kt:} Dettaglio profilo amico con intestazione, statistiche di lettura, libri in corso e ultimi completati; include pulsante per rimuovere l’amicizia con dialogo di conferma e callback; layout composto da \textit{FriendProfileHeader}, \textit{ReadingStatisticsCard}, \textit{CurrentReadingBooksCard}, \textit{CompletedBooksCard}.
      \item \textbf{FriendComponents.kt:} Componenti riutilizzabili per la sezione amici: barra di ricerca e selezione schede (\textit{SearchAndTabs}), riga utente con azioni contestuali (\textit{FriendItem}) e riga richiesta con accetta o rifiuta (\textit{RequestItem}); gestisce stati “non amico”, “in attesa”, “amico” con icone e colori dedicati.
      \item \textbf{SupportScreen.kt:} Schermata informazioni e supporto con sezioni su versione app, contatti e policy; include area pericolosa per eliminazione account con doppio passaggio di conferma, reinserimento password e flusso di reautenticazione, rimozione dati e cancellazione account; dopo esito positivo invoca callback di uscita.
    \end{itemize}
  \end{itemize}
  \item \textbf{catalog:}
  \begin{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item \textbf{ui:}
    \begin{itemize}
      \item \textbf{login-screen.dart:} Schermata di accesso costruita con \textit{Scaffold} e \textit{AppBar}. Il corpo contiene una \textit{ListView} con due \textit{TextField} (email e password), un \textit{FilledButton} che richiama \textit{submitLogin} e mostra un \textit{CircularProgressIndicator} quando \textit{isSubmitting}, più un \textit{TextButton} per passare alla registrazione (\textit{onRegisterClick}). Gli errori di validazione/server sono mostrati in testa; l’abilitazione del bottone dipende da \textit{canSubmit}. Lo stato e le azioni provengono da \textit{AuthViewModel} tramite \textit{Provider} (\textit{context.watch<AuthViewModel>()}); al successo invoca \textit{onLoginSuccess}.
      \item \textbf{registration-screen.dart:} Schermata di registrazione strutturata con \textit{Scaffold} e \textit{AppBar}. Include \textit{TextField} per nome, cognome, username, email, password e conferma; sotto lo username mostra un messaggio dinamico basato su \textit{usernameAvailable} (colore coerente con il tema). Il \textit{FilledButton} richiama \textit{submitRegister}, mostra lo spinner durante \textit{isSubmitting} ed è abilitato solo se \textit{canSubmit}; al completamento chiama \textit{onRegistered}. Presente un \textit{TextButton} per tornare al login (\textit{onLoginClick}) e un’area per eventuali errori. Stato e metodi sono forniti da \textit{AuthViewModel} tramite \textit{Provider}.
    \end{itemize}
    \item \textbf{viewmodel/auth-view-model.dart:} \textit{AuthViewModel} estende \textit{ChangeNotifier} e incapsula la logica di autenticazione usando \textbf{AuthRepository}. Espone due stati immutabili, \textit{LoginUiState} e \textit{RegisterUiState}, con metodi di aggiornamento dei campi e flag (\textit{isSubmitting}, \textit{success}, \textit{error}). Fornisce lo stream \textit{authState} e le azioni: \textit{submitLogin} (chiama \textit{repo.login}, sincronizza amici con \textit{FriendsManager.syncMyFriendsFromAcceptedRequests}, gestisce errori \textit{FirebaseAuthException}) e \textit{submitRegister} (chiama \textit{repo.register} con gestione di codice \textit{username-taken}). La disponibilità dello username è verificata con debounce (\textit{Timer} a 400 ms) in \textit{onUsernameChange}. Presente \textit{logout} e \textit{dispose} per annullare il timer.
  \end{itemize}
  \item \textbf{features/bookdetail:}
  \begin{itemize}
    \item \textbf{ui/widgets/reading-dialogs.dart:} Definisce due dialog numerici riutilizzabili: \textit{TotalPagesDialog} (imposta il totale pagine) e \textit{ReadPagesDialog} (aggiorna le pagine lette, opzionalmente mostrando “(d)/(max)”). Entrambi usano \textit{TextField} con filtro ai soli numeri, validazione con \textit{errorText}, e pulsanti \textit{Annulla}/\textit{Salva} che abilitano la conferma solo se l’input è valido.
    \item \textbf{ui/book-detail-screen.dart:} Schermata dettagli libro \textit{BookDetailScreen} con intestazione (copertina \textit{BookCover}, titolo, autori, metadati) e descrizione espandibile \textit{ExpandableText}. Integra la barra di stato \textit{BookStatusBar} (icone “Da leggere”, “In lettura”, “Letto” con disabilitazione quando \textit{busy}) e orchestra i flussi \textit{ReadingFlowMode start/update}: se mancano le pagine totali chiede il totale, poi le pagine lette; in caso “Letto” imposta le pagine lette uguali al totale. Si collega al \textit{BookDetailViewModel} (\textit{bindVolume} in \textit{initState}), ascolta \textit{events} per mostrare \textit{SnackBar} e invoca \textit{setStatusWithPages}/\textit{clearStatus}; include un helper \textit{askPositiveInt} per prompt numerici rapidi.
    \item \textbf{viewmodel/book-detail-view-model.dart:} \textit{ChangeNotifier} che media tra UI e Firestore per un volume. Espone stato osservabile: \textit{status} (enum \textit{ReadingStatus}), \textit{savedReadPages}, \textit{savedTotalPages}, flag \textit{isBusy} ed \textit{events} (stream di messaggi UI). \textit{bindVolume} sottoscrive il documento \textit{users/{uid}/books/{volumeId}} sincronizzando campi \textit{status}/\textit{pageInReading}/\textit{pageCount}. \textit{setStatusWithPages} esegue un upsert atomico (\textit{SetOptions.merge}) scrivendo metadati libro, \textit{status}, eventuali \textit{pageCount}/\textit{pageInReading} e timestamp server; aggiorna lo stato locale ed emette un evento descrittivo. Fornisce \textit{onStatusIconClick} (toggle: riclick sullo stesso stato rimuove il libro), \textit{clearStatus}, \textit{updatePageCount}, \textit{updatePagesRead}; ricava l’\textit{uid} da \textit{FirebaseAuth} e usa \textit{FirebaseFirestore}. Pulizia in \textit{dispose} (chiusura stream e subscription).
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      Schermata di dettaglio per un amico: mostra intestazione con profilo, statistiche aggregate di lettura e due sezioni libri (\textit{in lettura} e \textit{completati}). Carica dati e libreria dell’utente selezionato solo se è nella propria lista amici, offre rimozione amicizia con conferma e formatta i tempi di lettura in modo leggibile.
      \item \textbf{friends-list-screen.dart:}
      Elenco amici con tre tab (\textit{Amici}, \textit{Richieste}, \textit{Utenti}), ricerca testuale, stato di caricamento ed errori. Consente invio richieste, accettazione o rifiuto, sincronizzazione automatica delle amicizie e navigazione al dettaglio amico. Usa elementi lista dedicati per utenti e richieste e aggiorna la vista in base ai filtri selezionati.
      \item \textbf{profile-screen.dart:}
      Hub del profilo utente con sottopagine \textit{Profilo}, \textit{Dati personali}, \textit{Modifica dati}, \textit{Info e supporto}. Integra il view model per leggere e aggiornare i dati, gestisce successo ed errori di salvataggio, logout ed eliminazione account. Dalla pagina principale apre lista amici e dettaglio amico; intercetta il tasto indietro per tornare ordinatamente alla vista precedente.
    \end{itemize}
    \item \textbf{viewmodel/profile-view-mode.dart:}
    View model del profilo basato su \textit{ChangeNotifier}: carica il profilo dell’utente all’inizializzazione tramite il \textit{repository}, espone stato osservabile (utente, caricamento, caricamento aggiornamento, esito aggiornamento), e fornisce metodi per aggiornare i dati personali. Il metodo \textit{updateUserProfile} chiama il \textit{repository}, aggiorna la cache locale, gestisce gli esiti (successo, username non disponibile, errore) e notifica la UI; \textit{clearUpdateResult} azzera l’esito per successive operazioni.
  \end{itemize}
  \item \textbf{features/shelves:}
  \begin{itemize}
    \item \textbf{model:}
    \begin{itemize}
      \item \textbf{reading-status.dart:}
      Definisce l’\textit{enumerazione} degli stati di lettura (Da leggere, In lettura, Letto) e i \textit{metodi di utilità} per convertirli: da stringa a stato, da stato a etichetta per la UI, e da stato a codice persistito (valori testuali come “TO READ”, “READING”, “READ”), oltre alla conversione inversa dai codici allo stato. Garantisce coerenza tra livello dominio, interfaccia e salvataggio su Firestore.
